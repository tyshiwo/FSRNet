...
if type_ == 'mse' then
    -- Helen_1.8W_MSE_8x
    model = torch.load('./models/Helen_8x.t7')  
elseif type_ == 'gan' then
    -- Helen_1.8W_GAN_8x
    model = torch.load('./models/Helen_GAN_8x.t7')    
end
...
for line in file:lines() do
    ...
    if type_ == 'mse' then
        image.save('./results/helen_fsrnet_8x/output_2_'.. filename_,image_save)
    elseif type_ == 'gan' then
        image.save('./results/helen_fsrgan_8x/output_2_'.. filename_,image_save)
    end
